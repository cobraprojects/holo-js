...
describe('auth documentation smoke checks', () => {
  ...
  it('covers local auth, guards, and token APIs', async () => {
    ...
    expect(tokens).toContain('auth.guard(\'api\').login')
    expect(tokens).toContain('auth.guard(\'api\').register')
  ...
  it('covers current-user client helpers and lifecycle-token delivery through notifications and mail', async () => {
    ...
    expect(verification).toContain('verifyEmail')
    expect(verification).toContain('resendEmailVerification')
