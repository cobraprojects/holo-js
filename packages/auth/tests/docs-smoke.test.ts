...
describe('auth documentation smoke checks', () => {
  ...
  it('covers current-user client helpers and lifecycle-token delivery through notifications and mail', async () => {
    ...
    expect(verification).toContain('emailVerificationRequired')
    expect(verification).toContain('emailVerificationRoute')
    expect(verification).toContain('verification.consume')
    expect(verification).toContain('verification.resend')
    ...
    expect(reset).not.toContain('passwords.request')
    expect(reset).not.toContain('passwords.consume')
