...
async function writePluginPackage(
  ...
  await writeFile(join(packageRoot, 'plugin.mjs'), entryContents)
  return packageRoot
}
...
describe('@holo-js/config', () => {
  ...
  it('normalizes database defaults and freezes media, mail, notifications, queue, and security config values', () => {
    ...
    const normalizedQueueWithNamedRedisConnection = normalizeQueueConfigForHolo({
      connections: {
        redis: {
          driver: 'redis',
          queue: 'emails',
    ...
    expect(normalizedQueueWithNamedRedisConnection.connections).toMatchObject({
      redis: {
        driver: 'redis',
        connection: 'cache',
        queue: 'emails',
    ...
    const normalizedQueueWithRedisUrlConnection = normalizeQueueConfigForHolo({
      failed: {
        driver: 'database',
      },
      connections: {
    ...
    expect(normalizedQueueWithRedisUrlConnection).toMatchObject({
      failed: {
        connection: 'default',
        table: 'failed_jobs',
      },
    ...
    const normalizedQueueWithSharedDefaultRedis = normalizeQueueConfigForHolo({
      connections: {
        redis: {
          driver: 'redis',
        },
    ...
    expect(normalizedQueueWithSharedDefaultRedis).toMatchObject({
      connections: {
        redis: {
          connection: 'cache',
          redis: {
    ...
    expect(normalizedQueueWithBlankSharedRedisCredentials).toMatchObject({
      failed: {
        connection: 'default',
        table: 'failed_jobs',
      },
    ...
    const pluginQueueConfig = normalizeQueueConfigForHolo({
      connections: {
        sqs: {
          driver: 'sqs' as never,
          queue: 'mail',
    ...
    expect(pluginQueueConfig.connections.sqs).toMatchObject({
      name: 'sqs',
      driver: 'sqs',
      queue: 'mail',
      visibilityTimeout: 30,
  ...
  it('rejects unresolved shared Redis connections for Redis-backed security rate limiting', () => {
...
`, 'utf8')
    await writeFile(join(root, 'config/secondary.ts'), 'export default { value: "primary" }', 'utf8')
    await writeFile(join(root, 'config/secondary.mjs'), 'export default { value: "secondary" }', 'utf8')
    await writeFile(join(root, 'config/ordered.js'), 'export default { value: "primary" }', 'utf8')
    await writeFile(join(root, 'config/ordered.mjs'), 'export default { value: "secondary" }', 'utf8')
    await loadConfigDirectory(join(root, 'missing'), {
      processEnv: {},
      preferCache: false,
    })
...
`, 'utf8')
  ...
  it('normalizes session and auth defaults and freezes their config helpers', () => {
    ...
    const normalizedRedisSession = normalizeSessionConfig({
      stores: {
        cache: {
          driver: 'redis',
          connection: 'cache',
    ...
    expect(normalizedRedisSession.driver).toBe('cache')
    expect(normalizedRedisSession.stores.cache).toMatchObject({
      name: 'cache',
      driver: 'redis',
      connection: 'cache',
      url: 'redis://cache.internal:6380/4',
