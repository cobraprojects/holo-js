...
describe('cache documentation smoke checks', () => {
  it('covers install flow, drivers, and shared config requirements', async () => {
    ...
    expect(setup).toContain('bunx holo install cache')
    expect(setup).toContain('bunx holo install cache --driver redis')
    expect(setup).toContain('bunx holo install cache --driver database')
    expect(setup).toContain('bunx holo cache:table')
    expect(setup).toContain('bunx holo cache:clear')
    expect(setup).toContain('bunx holo cache:forget dashboard.stats')
  ...
  it('covers runtime usage, locking, and query invalidation behavior', async () => {
    ...
    expect(commands).toContain('bunx holo cache:table')
    expect(commands).toContain('bunx holo cache:clear')
    expect(commands).toContain('bunx holo cache:forget dashboard.stats')
  })
})
