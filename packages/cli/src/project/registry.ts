...
export function renderGeneratedAuthTypes(
  guards: Readonly<Record<string, { readonly driver: string }>>,
): string {
  const members = Object.entries(guards)
    .sort(([left], [right]) => left.localeCompare(right))
    .filter((entry): entry is [string, { readonly driver: 'session' | 'token' }] => {
...
export async function writeGeneratedProjectRegistry(
  ...
  await writeFileIfChanged(resolve(projectRoot, GENERATED_AUTH_TYPES_PATH), renderGeneratedAuthTypes(loadedConfig?.auth?.guards ?? {}))
