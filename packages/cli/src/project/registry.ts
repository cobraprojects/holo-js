...
import type { NormalizedHoloAuthConfig } from '@holo-js/auth'
import { loadConfigDirectory } from '@holo-js/config'
import { renderGeneratedSchemaRuntimeModule, type TableDefinition } from '@holo-js/db'
...
export function renderGeneratedChannelImporter(channels: readonly GeneratedChannelRegistryEntry[]): string {
  const importableChannels = channels.filter(entry => ['.ts', '.mts', '.cts', '.js', '.mjs', '.cjs'].includes(extname(entry.sourcePath)))
...
type AuthTypeModelRegistryEntry = Pick<GeneratedModelRegistryEntry, 'name' | 'tableName'>

function getAuthProviderModelNames(
  auth: Pick<NormalizedHoloAuthConfig, 'guards' | 'providers'> | undefined,
): readonly string[] {
  if (!auth) {
    return []
  }

  return [...new Set(Object.values(auth.guards)
    .map(guard => auth.providers[guard.provider]?.model)
    .filter((model): model is string => typeof model === 'string' && model.length > 0))]
    .sort((left, right) => left.localeCompare(right))
}

export function renderGeneratedAuthTypes(
  auth: Pick<NormalizedHoloAuthConfig, 'guards' | 'providers'> | undefined,
  models: readonly AuthTypeModelRegistryEntry[] = [],
): string {
  const guards = auth?.guards ?? {}
  const modelByName = new Map(models.map(model => [model.name, model]))
  const userModels = getAuthProviderModelNames(auth)
    .map(modelName => modelByName.get(modelName))
    .filter((model): model is AuthTypeModelRegistryEntry => Boolean(model))
  const userTableNames = [...new Set(userModels.map(model => model.tableName))]
  const userType = userModels.length > 0
    ? userTableNames.map(tableName => `InferSelect<GeneratedSchemaTable<${JSON.stringify(tableName)}>>`).join(' | ')
    : undefined
  const imports = userType
    ...
    : []
  const members = Object.entries(guards)
    .sort(([left], [right]) => left.localeCompare(right))
    .filter((entry) => {
...
export async function writeGeneratedProjectRegistry(
  ...
  await writeFileIfChanged(resolve(projectRoot, GENERATED_MODEL_TYPES_PATH), renderGeneratedModelTypes(nextRegistry.models))
  ...
  await writeFileIfChanged(resolve(projectRoot, GENERATED_CHANNEL_IMPORTER_PATH), renderGeneratedChannelImporter(nextRegistry.channels))
  ...
  await writeFileIfChanged(resolve(projectRoot, GENERATED_AUTH_TYPES_PATH), renderGeneratedAuthTypes(loadedConfig?.auth, nextRegistry.models))
