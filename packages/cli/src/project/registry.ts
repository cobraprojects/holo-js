...
export function renderGeneratedIndexModule(): string {
  return [
    ...
    '',
    'export { metadata, models, migrations, seeders, commands, jobs, events, listeners, broadcast, channels, broadcastManifest }',
    '',
...
export function renderGeneratedBroadcastTypes(
  ...
  const broadcastMembers = broadcast.map((entry) => {
    ...
    if (!importName || !entry.exportName) {
      return `    ${JSON.stringify(entry.name)}: import('@holo-js/broadcast').BroadcastDefinition`
    }

    return `    ${JSON.stringify(entry.name)}: import('@holo-js/broadcast').ExportedBroadcastDefinition<typeof ${importName}[${JSON.stringify(entry.exportName)}]>`
  })

  const channelMembers = channels.map((entry) => {
    const importName = channelImportNameByPattern.get(entry.pattern)
    if (!importName || !entry.exportName) {
      return `    ${JSON.stringify(entry.pattern)}: import('@holo-js/broadcast').ChannelDefinition`
    }

    return `    ${JSON.stringify(entry.pattern)}: import('@holo-js/broadcast').ExportedChannelDefinition<typeof ${importName}[${JSON.stringify(entry.exportName)}]>`
  })
...
export function renderGeneratedBroadcastManifest(
  ...
  const channelLines = registry.channels.flatMap((entry, index) => {
    ...
    if (entry.type === 'presence') {
      lines.push(
        `      member: undefined as unknown as import('@holo-js/broadcast').ChannelPresenceMemberFor<${JSON.stringify(entry.pattern)}>,`,
      )
    }
  ...
  return [
    ...
    ...channelLines,
    '  ],',
    '} as const satisfies import(\'@holo-js/broadcast\').GeneratedBroadcastManifest',
    '',
...
function renderGeneratedConfigTypes(
  ...
  const customEntries = entries.filter(entry => !['app', 'database', 'storage', 'queue', 'broadcast', 'notifications', 'mail', 'media', 'session', 'auth'].includes(entry.configName))
