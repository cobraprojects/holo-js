...
export function renderFrameworkRunnerForDescriptor(descriptor: FrameworkDescriptor): string {
  ...
  const commandPackageNames = [
    ...Object.keys(descriptor.scaffold.dependencies),
    ...Object.keys(descriptor.scaffold.devDependencies),
  ]
  return [
    ...
    `const commandPackageNames = ${JSON.stringify(commandPackageNames)}`,
    'const passthroughArgs = process.argv.slice(3)',
    'const serverEnv = {}',
    'const nodeModulesPath = resolve(projectRoot, \'node_modules\')',
    'const binaryPath = resolve(nodeModulesPath, \'.bin\', commandName)',
    '',
    ...
    '  for (const packageName of commandPackageNames) {',
    '    const manifestPath = resolve(nodeModulesPath, packageName, \'package.json\')',
    '    const relativeManifestPath = relative(nodeModulesPath, manifestPath)',
    '    if (relativeManifestPath.startsWith(\'..\') || isAbsolute(relativeManifestPath) || !existsSync(manifestPath)) {',
    '      continue',
    '    }',
    '',
    ...
    '      ? packageManifest.bin',
    '      : packageManifest.bin?.[commandName]',
    '    if (typeof bin !== \'string\') {',
    '      continue',
    '    }',
    '',
    '    const packagePath = dirname(manifestPath)',
    '    const commandPath = resolve(packagePath, bin)',
    '    const relativeCommandPath = relative(packagePath, commandPath)',
    '    if (!relativeCommandPath.startsWith(\'..\') && !isAbsolute(relativeCommandPath) && existsSync(commandPath)) {',
    '      return commandPath',
    '    }',
    '  }',
    '',
    ...
    '}',
    '',
    'const windowsCommandPath = process.platform === \'win32\' ? resolveWindowsCommandPath() : undefined',
    '',
    'function resolveFrameworkBinaryInvocation(args) {',
    '  return process.platform === \'win32\'',
    '    ? { command: process.execPath, args: windowsCommandPath ? [windowsCommandPath, ...args] : [] }',
    '    : { command: binaryPath, args }',
    '}',
    '',
    'function resolveCommandInvocation() {',
    '  if (mode === \'dev\') {',
    '    return resolveFrameworkBinaryInvocation([\'dev\', ...passthroughArgs])',
    '  }',
    '',
    '  if (mode === \'build\') {',
    '    return resolveFrameworkBinaryInvocation([...runner.buildArgs, ...passthroughArgs])',
    '  }',
    '',
    '  if (mode === \'start\') {',
    '    if (runner.startUsesFrameworkBinary) {',
    '      return resolveFrameworkBinaryInvocation([...runner.start, ...passthroughArgs])',
    '    }',
    '',
    '    const serverArgs = []',
    '    for (let index = 0; index < passthroughArgs.length; index += 1) {',
    '      const arg = passthroughArgs[index]',
    '      if ((framework !== \'nuxt\' && framework !== \'sveltekit\') || (arg !== \'--port\' && !arg.startsWith(\'--port=\'))) {',
    '        serverArgs.push(arg)',
    '        continue',
    '      }',
    '',
    '      const port = arg === \'--port\' ? passthroughArgs[++index] : arg.slice(\'--port=\'.length)',
    '      if (!port || !/^\\d+$/.test(port) || Number(port) > 65535) {',
    '        throw new Error(\'[holo] --port must be an integer between 0 and 65535.\')',
    '      }',
    '      serverEnv.PORT = port',
    '      if (framework === \'nuxt\') {',
    '        serverEnv.NITRO_PORT = port',
    '      }',
    '    }',
    '',
    '    const [entry, ...args] = runner.start',
    '    return { command: process.execPath, args: [resolveProjectPath(entry), ...args, ...serverArgs] }',
    '  }',
    '',
    '  return undefined',
    '}',
    ...
    '}',
    '',
    'const frameworkBinaryExists = process.platform === \'win32\'',
    '  ? typeof windowsCommandPath === \'string\'',
    '  : existsSync(binaryPath)',
    '',
    'if ((mode !== \'start\' || runner.startUsesFrameworkBinary) && !frameworkBinaryExists) {',
