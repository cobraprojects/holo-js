...
function readCode(error: Record<string, unknown>): string | undefined {
...
export function normalizeHoloHttpError(error: unknown): NormalizedHoloHttpError | undefined {
  ...
  return {
    status,
    message: readMessage(error as Record<string, unknown>),
    code: readCode(error as Record<string, unknown>),
    cause: error,
  }
}
