...
describe('multi-dialect schema compilers', () => {
  ...
  it('lowers logical schema types to MySQL syntax and fails closed for unsupported vectors', () => {
    ...
    expect(statements[0]!.sql).toContain('`snowflake` VARCHAR(32) NOT NULL')
    expect(statements[0]!.sql).toContain('`account_uuid` CHAR(36) NOT NULL,')
    expect(statements[0]!.sql).toContain('`session_ulid` CHAR(26) NOT NULL,')
    expect(statements[0]!.sql).toContain('`actor_snowflake` VARCHAR(32) NOT NULL,')
    expect(statements[0]!.sql).toContain('CONSTRAINT `users_account_uuid_foreign` FOREIGN KEY (`account_uuid`) REFERENCES `accounts` (`uuid`)')
    expect(statements[0]!.sql).toContain('CONSTRAINT `users_session_ulid_foreign` FOREIGN KEY (`session_ulid`) REFERENCES `sessions` (`id`)')
    expect(statements[0]!.sql).toContain('CONSTRAINT `users_actor_snowflake_foreign` FOREIGN KEY (`actor_snowflake`) REFERENCES `actors` (`snowflake_id`)')
    expect(statements[0]!.sql).toContain('`startsOn` DATE NOT NULL DEFAULT \'2025-01-02T03:04:05.000Z\'')
