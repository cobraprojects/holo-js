...
export function renderNuxtClientHttpErrorPage(error: NormalizedHoloHttpError): void {
  const nuxtGlobal = globalThis as NuxtClientGlobal
  const nuxtError = {
  ...
  if (typeof nuxtGlobal.showError === 'function') {
    nuxtGlobal.showError(nuxtError)
    return
  }

  renderFallbackClientHttpErrorPage(error)
}

function renderFallbackClientHttpErrorPage(error: NormalizedHoloHttpError): void {
  ...
  const existingRoot = browserDocument.getElementById('__holo_nuxt_client_http_error__')
  const root = existingRoot ?? browserDocument.createElement('main')
  root.id = '__holo_nuxt_client_http_error__'
  ...
  status.className = 'nuxt-error-h1'
