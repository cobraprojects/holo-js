...
describe('@holo-js/adapter-nuxt package boundaries', () => {
  it('keeps exported storage and forms surfaces as optional peers', async () => {
    ...
    expect(moduleEntry).not.toContain("@holo-js/forms")
    expect(moduleEntry).toContain("import('@holo-js/storage')")
    expect(moduleEntry).toContain("import('@holo-js/storage-s3' as string)")
    ...
    expect(storagePlugin).toContain("@holo-js/storage/runtime")
    expect(storageRoute).toContain("@holo-js/storage")
    expect(s3Driver).toContain("@holo-js/storage-s3")
