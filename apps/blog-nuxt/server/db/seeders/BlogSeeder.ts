...
export default defineSeeder({
  ...
  async run() {
    ...
    const nuxtTag = await Tag.create({ name: 'nuxt', slug: 'nuxt' })
    const releaseTag = await Tag.create({ name: 'release', slug: 'release' })

    const launchPost = await Post.create({
      user_id: author.id,
      category_id: engineering.id,
      title: 'Shipping a Real Holo Blog on Nuxt',
      slug: 'shipping-a-real-holo-blog-on-nuxt',
      status: 'published',
    ...
    await launchPost.tags().attach([frameworkTag.id, nuxtTag.id])
