...
Flux loads browser-safe connection settings from `/broadcasting/config` and uses `/broadcasting/auth` for private and
presence channel authorization.
Both endpoints are generated by `holo install broadcast`.
