...
export default defineSeeder({
  ...
  async run() {
    ...
    const svelteTag = await Tag.create({ name: 'sveltekit', slug: 'sveltekit' })
    const releaseTag = await Tag.create({ name: 'release', slug: 'release' })

    const launchPost = await Post.create({
      user_id: author.id,
      category_id: engineering.id,
      title: 'Shipping a Real Holo Blog on SvelteKit',
      slug: 'shipping-a-real-holo-blog-on-sveltekit',
      status: 'published',
    ...
    await launchPost.tags().attach([frameworkTag.id, svelteTag.id])
